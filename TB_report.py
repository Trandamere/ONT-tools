...
# 设定监控日志输出文件名和内容形式
logging.basicConfig(format='%(asctime)s - %(message)s', filename='/mnt/c/Users/luping/Desktop/报告流程/test/运行信息.txt', filemode='a', level=logging.INFO)

# 参数的导入与处理
parser = argparse.ArgumentParser()
parser.add_argument('-i', "--result_excel", required=True, help="the excel file with the result selected")
parser.add_argument('-b', "--database", type=str, default='/mnt/c/Users/luping/Desktop/报告流程/test/思可愈数据库-TNP-Seq病原菌测序项目2021.05.18.xlsx',help="database provided by the Ministry of Medicine")
# parser.add_argument('-w', "--word_template_folder", type=str, default='/home/MicroReports/pipelines/TB-seq/分枝杆菌(MTB&NTM)及其耐药基因检测报告.docx',help="folder where all word report templates are located")
...
parser.add_argument('-c', "--complex_excel", type=str, default='/mnt/c/Users/luping/Desktop/报告流程/test/mycobacterium_tuberculosis_complex.xlsx',help="结核分枝杆菌复合群包含微生物表格")
# parser.add_argument('-s', "--summary_excel", type=str, default='/home/MicroReports/pipelines/TB-seq/OUTPUT/',help="summary documents before processing")
parser.add_argument('-m', "--name_excel", type=str, default='/mnt/c/Users/luping/Desktop/报告流程/test/药品中英文对照表.xlsx',help="检测药品中英文对照表")
parser.add_argument('-o', "--output_dir", type=str, default='/mnt/c/Users/luping/Desktop/报告流程/test/OUTPUT/',help="supplement sample result")
...
for sample_code in dic_client.keys():
    ...
    if '耐药基因' in project:
        ...
        doc = DocxTemplate(f'/mnt/c/Users/luping/Desktop/报告流程/test/分枝杆菌(MTB&NTM)及其耐药基因检测报告.docx')
        ...
        logging.info(f"{result_report_name}开始生成")
        doc = DocxTemplate(f'/home/MicroReports/pipelines/TB-seq/{agent}/TB_seq结核&非结核分枝杆菌鉴定.docx')
        print(dic_client[sample_code])
        doc.render(dic_client[sample_code])
        color_change(doc=doc,table_number=table_number)
